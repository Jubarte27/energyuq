...
def parse_args():
    ...
    parser.add_argument(
        "--individual",
        action="store_true",
        help="Perform individual analysis in bulk for each discovered run.",
    )
    parser.add_argument(
        "--individual-dir",
        type=str,
        default=None,
        help="Output directory for individual run analyses (defaults to <output-dir>/individual).",
    )
    parser.add_argument(
        "--show",
        action="store_true",
...
def main():
    ...
    if args.plots.lower() != "none":
        ...
        if generate_all or "sobols" in plot_choices:
            ...
            figs["sobol_indices_grouped_bar"] = multi_plot.plot_multi_sobols(runs, qoi=args.qoi, mode="grouped_bar", show_title=False)
            figs["sobol_indices_heatmap"] = multi_plot.plot_multi_sobols(runs, qoi=args.qoi, mode="heatmap", show_title=False)

        # Convergence
        if generate_all or "convergence" in plot_choices:
            if not args.quiet:
                print("Generating convergence history plot...")
            figs["convergence_history_by_benchmark"] = multi_plot.plot_multi_convergence(runs, metric="adaptation_error", facet_by="benchmark", show_title=False)

        # Energy vs Time Pareto
        if generate_all or "pareto" in plot_choices:
            if not args.quiet:
                print("Generating Pareto frontier plot...")
            figs["pareto_frontier_energy_time"] = multi_plot.plot_multi_energy_time_pareto(runs, qoi_x="time", qoi_y=args.qoi, show_title=False)

        # QoI Distributions
        if generate_all or "distribution" in plot_choices:
            if not args.quiet:
                print("Generating QoI distribution boxplots...")
            figs["qoi_distribution_by_machine"] = multi_plot.plot_multi_qoi_distribution(runs, qoi=args.qoi, group_by="machine", show_title=False)
            figs["qoi_distribution_by_benchmark"] = multi_plot.plot_multi_qoi_distribution(runs, qoi=args.qoi, group_by="benchmark", show_title=False)

        # Best configurations
        if generate_all or "best" in plot_choices:
            if not args.quiet:
                print("Generating optimal configurations plot...")
            figs["optimal_configurations"] = multi_plot.plot_multi_best_configurations(runs, qoi=args.qoi, show_title=False)

        # Parameter effects
        if generate_all or "effects" in plot_choices:
            if not args.quiet:
                print("Generating parameter effects plots...")
            figs["parameter_effects_threads"] = multi_plot.plot_multi_parameter_effects(runs, param="N_THREADS", qoi=args.qoi, show_title=False)

        # Dashboard
        if generate_all or "dashboard" in plot_choices:
            if not args.quiet:
                print("Generating comprehensive summary dashboard...")
            figs["multi_run_dashboard"] = multi_plot.plot_multi_dashboard(runs, qoi=args.qoi, show_title=False)
    ...
    # 3. Individual run analyses in bulk
    if args.individual:
        indiv_dir = Path(args.individual_dir) if args.individual_dir else (out_dir / "individual")
        if not args.quiet:
            print(f"\n--- Running Bulk Individual Analysis for {len(runs)} runs into '{indiv_dir}' ---")
        qois_to_analyze = [args.qoi]
        if "time" not in qois_to_analyze:
            qois_to_analyze.append("time")
        runs.analyze_each(
            output_dir=indiv_dir,
            qois=qois_to_analyze,
            save_plots=(args.plots.lower() != "none"),
            fmt=args.format,
            dpi=args.dpi,
            quiet=args.quiet,
        )
